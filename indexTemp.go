...
func getClientItems(w http.ResponseWriter, r *http.Request) {
	...
	if lastRun == 0 {
		// first run, get everything updated in the last min
		lastRun = time.Now().Add(time.Minute * -1).Unix()
	}
	var auctions = callHypixelAuctions(lastRun)
		...
		if a.Bin && parse {
			...
			if lowestBins[lbName] != nil {
				...
				if lowestBin > 0 {
					...
					if av.Profit > minProfit && (enchantments || (!enchantments && a.ItemName != "Enchanted Book")) {
